...
with st.sidebar:
    selected = option_menu("Menu",["EMI","PPF","FD","EPF"],
        icons=['house','cloud','calculator','calculator-fill'], menu_icon="cast", default_index=0)
if selected=='EMI':
  ...
  def main():
    Loan = st.slider('Select Loan Amount in lakhs', 0, 200, 50)
    #st.write("Loan amount:", Years)
    Years=st.slider('Select Tenure Period in Years', 0, 25,10)
    Rate=st.slider('Select Rate of Interest', 0.0,15.0,6.0,step=0.25)
    st.write("-----------------------")
    Loan=Loan*100000
    PR=Rate/1200
